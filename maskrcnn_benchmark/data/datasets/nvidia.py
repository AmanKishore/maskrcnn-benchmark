...
def nvidia_to_coco(image_dir, dataset):
    # as you would do normally

    new_dir = os.path.join(image_dir, "coco_format")
    new_images = os.path.join(image_dir, "coco_format", "images")
    ...
    json_file["categories"].append(dict(supercategory='automobile', id=1, name='automobile'))
    json_file["categories"].append(dict(supercategory='bicycle', id=2, name='bicycle'))
    json_file["categories"].append(dict(supercategory='person', id=3, name='person'))
    json_file["categories"].append(dict(supercategory='heavy_truck', id=4, name='heavy_truck'))
    datasetv9 = False
    image_id = 1
    object_id = 1

    for img_path in glob.glob(image_dir + '/frames/**', recursive=True):
        if os.path.splitext(img_path)[1] in {'.fp16', '.jpeg', 'jpg', '.png'}:
            ...
            try: conn = sqlite3.connect(dataset)
            ...
            new_img_path = os.path.join(new_images, os.path.splitext(img_basename)[0] + '.jpeg')
            print(new_img_path)
            image.save(new_img_path)

            json_file["licenses"].append(
                {
            ...
            json_file["images"].append(
                {
                    "license": 1,
                    "file_name": os.path.splitext(img_basename)[0] + '.jpeg',
                    "coco_url": "http://www.nvidia.com",
                    "height": height,
                    "width": width,
                    "date_captured": "2021-05-01 17:02:52",
                    "flickr_url": "http://www.nvidia.com",
                    "id": image_id
                }
            )
            image_id += 1
    ...
    json_path = os.path.join(image_dir, "coco_format", "dataset-v9.json")
    with open(json_path, 'w') as outfile:
        json.dump(json_file, outfile)

image_dir = "/home/scratch.nvdrivenet/amkishore/FrontSim_Data/drivesimdataKPI/up/"
dataset = "/home/amkishore/drivesimdataKPI/dataset-v9.sqlite"
nvidia_to_coco(image_dir, dataset)
